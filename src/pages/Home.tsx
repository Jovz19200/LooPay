...
import {
  Building2, Clock, EyeOff, ShoppingCart,
  CreditCard, Smartphone, Store,
  Signal, Landmark, Rocket, Lightbulb,
} from 'lucide-react';
...
const problemCards = [
  { icon: <Building2 size={20} />, title: 'Cash held by admin',  body: 'Students hand over all money to school administration. No receipt, no guarantee it will be returned in full.', accent: 'var(--red)' },
  { icon: <Clock       size={20} />, title: 'Access denied',      body: 'Students must justify every withdrawal to an administrator — and still wait days for their own money.', accent: 'var(--orange)' },
  { icon: <EyeOff      size={20} />, title: 'Parents are blind',  body: 'Parents send money with no confirmation it arrived. No balance visibility, no spending transparency.', accent: 'var(--yellow)' },
  { icon: <ShoppingCart size={20}/>, title: 'Nowhere to spend',   body: 'Even students with funds cannot buy basic items on campus. No cashless infrastructure exists.', accent: 'var(--purple)' },
];

const pillars = [
  {
    num: '01', icon: <CreditCard size={32} />, cls: 'pillar-blue',
    title: 'Digital Student Wallet',
    body: 'Each student receives an RFID/NFC card linked to a digital wallet. PIN-protected, works offline, no smartphone required.',
    points: ['Real-time balance updates', 'PIN protection on every transaction', 'Card costs under $1 to issue'],
  },
  {
    num: '02', icon: <Smartphone size={32} />, cls: 'pillar-green',
    title: 'Parent Mobile Top-Up',
  ...
  {
    num: '03', icon: <Store size={32} />, cls: 'pillar-orange',
...
const whyCards = [
  { icon: <Signal   size={22} />, bg: 'var(--blue-l)',   color: 'var(--blue)',   title: 'Infrastructure ready',    body: '77% of Rwandan adults have mobile money wallets. MTN MoMo API is open to third-party developers. The payment rails already exist.' },
  { icon: <Landmark size={22} />, bg: 'var(--green-l)',  color: 'var(--green)',  title: 'Government alignment',    body: "Rwanda's Vision 2050 and digital economy strategy actively support fintech innovation. BNR's 2023 payment regulation creates a clear licensing path." },
  { icon: <Rocket   size={22} />, bg: 'var(--orange-l)', color: 'var(--orange)', title: 'No competition yet',      body: 'Ugandan competitors (Kawallet, DesisPay, Xazu) have 62,000+ users but have not entered Rwanda. The window to be first is open.' },
  { icon: <Lightbulb size={22}/>, bg: 'var(--purple-l)', color: 'var(--purple)', title: 'Low MVP cost',            body: 'The entire proof-of-concept can be built and deployed for under $1,250. RFID cards cost $0.50 each. The technology is accessible and proven.' },
];
    ...
    <>
      ...
      <section className="section-light">
        <div className="container">
          ...
          <p className="section-sub">Boarding school students in Rwanda hand all their money to administrators on visiting days. What follows is a cascade of broken trust.</p>
      ...
      <CTASection
        ...
        sub="We are seeking co-founders, school partners, and seed investors to bring LooPeyi to Rwanda's boarding schools in 2026."
        buttons={[
